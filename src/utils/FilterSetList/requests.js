...
export async function getDomain () {
  ...
  const url = '/api/iac/getDomain'
  const response = await egFetch(url, myInit)
  const result = response || {status: 'Failed', data: '网络错误：获取域名失败，请联系管理员！'}
  console.log(result)
  return result
}
...
export async function saveAsNewFilterset (data) {
  ...
  const url = '/api/filterSet2/save/PurRadio'
...
export async function getConfig () {
  ...
  const url = '/api/filterSet2/getConfig2?dictList=&itemList=vendor,organization&module=PurRadio'
  const response = await egFetch(url, myInit)
...
export async function deleteScheme (name) {
  ...
  const url = `/api/filterSet/filterSets/${name}/PurRadio`
  const response = await egFetch(url, myInit)
